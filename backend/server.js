...
app.post("/github-webhook", async (req, res) => {
  ...
  try {
    if (ghEvent === "pull_request" && payload.action === "opened") {
      console.log(
        '[Webhook] Handling "pull_request" event with action "opened".'
      );
      ...
      console.log(
        `[PR Info] PR #${pullNumber} opened in ${owner}/${repoName} by ${prUserLogin}`
      );
      ...
      console.log(
        `[GitHub API] Retrieved ${changedFiles.length} changed files.`
      );
